...
def train(args):
    ...
    if checkpoint:
        ...
        print(f"Resuming from epoch {start_epoch} with best accuracy of {best_accuracy:.3f} so far")
    else:
        print("Starting training from the beginning")
        print(f"Test checkpoint is {args.test_checkpoint}")
    ...
    logger.debug(f"Processes {len(train_loader.sampler)}/{len(train_loader.dataset)} "
                 f"({100. * len(train_loader.sampler) / len(train_loader.dataset):.0f}%) of train data")

    logger.debug(f"Processes {len(test_loader.sampler)}/{len(test_loader.dataset)} "
                 f"({100. * len(test_loader.sampler) / len(test_loader.dataset):.0f}%) of test data")
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        assert (args.test_checkpoint != epoch), "Interrupting the training for checkpoint testing"
        is_best = False
        model.train()
        for batch_idx, (data, target) in enumerate(train_loader, 1):
            ...
            if batch_idx % args.log_interval == 0:
                logger.info(f'Train Epoch: {epoch} [{batch_idx * len(data)}/{len(train_loader.sampler)} '
    ...
    os.remove('/opt/ml/checkpoints/checkpoint.pth.tar')
...
def test(epoch, model, test_loader):
    ...
    logger.info(f'Epoch: {epoch}\tTest set: Average loss: {test_loss:.4f}, Accuracy: {100 * test_accuracy:.2f}%\n')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--test-checkpoint', type=int, default=0, metavar='N',
                        help='Enables container interrupt test for checkpoint restore at the Nth epoch (0 for no interruption)')
